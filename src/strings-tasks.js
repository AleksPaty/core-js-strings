...
function isString(value) {
  const primString =
    typeof value === 'object' && value != null ? value.valueOf() : false;
  return typeof value === 'string' || typeof primString === 'string';
}
...
function isPalindrome(str) {
  ...
  const reversStrArr = [];
  for (let i = str.length - 1; i >= 0; i -= 1) {
    reversStrArr.push(str.toLowerCase()[i]);
  }
  return str.toLowerCase() === reversStrArr.join('');
}
...
function findLongestWord(sentence) {
  const wordsArr = sentence.length > 0 ? sentence.split(' ') : '';
  let longestWord = '';
  let wordLength = 0;
...
function invertCase(str) {
  let invertStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const upperLet = str[i].toUpperCase();
    if (str[i] === upperLet) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] !== ' ' && alphabet.includes(str[i].toUpperCase())) {
      const letter = str[i];
      const letterInd = alphabet.indexOf(letter.toUpperCase());

      if (letterInd < 13) {
        encodeStr +=
          letter.toUpperCase() === letter
            ? alphabet[letterInd + 13]
            : alphabet[letterInd + 13].toLowerCase();
      } else {
        encodeStr +=
          letter.toUpperCase() === letter
            ? alphabet[letterInd - 13]
...
function getCardId(value) {
  ...
  const suitsArr = ['♣', '♦', '♥', '♠'];
  return (
    numArr.indexOf(value.slice(0, -1)) +
    suitsArr.indexOf(value[value.length - 1]) * numArr.length
  );
}
